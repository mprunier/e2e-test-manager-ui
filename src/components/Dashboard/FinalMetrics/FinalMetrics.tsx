...
const StatusIcon: FC<{ status: EPipelineStatus }> = ({ status }) => {
    switch (status) {
...
const MetricsDisplay: FC<{ data: IMetrics; onClick: () => void }> = ({ data, onClick }) => (
    <div
        onClick={onClick}
...
export const FinalMetrics: FC = () => {
    ...
    const isRunningAllTests = allTestsPipelines?.some((pipeline) => pipeline.status === EPipelineStatus.IN_PROGRESS);
                ...
                isRunningAllTests && (
                    <div className="mt-4 flex items-center justify-between rounded-lg bg-blue-50 p-3">
                        <strong className="text-sm text-gray-600">
                            All tests are currently being executed on {allTestsPipelines.length} pipeline
