...
interface IParams {
    ...
    onSortFieldChange: (field: SearchSuiteConfigurationSortField) => void;
    onSortOrderChange: (order: SearchSuiteConfigurationSortOrder) => void;
}

const sortFieldLabels: Record<SearchSuiteConfigurationSortField, string> = {
    [SearchSuiteConfigurationSortField.FILE]: "File Name",
    [SearchSuiteConfigurationSortField.TITLE]: "Suite Title",
    [SearchSuiteConfigurationSortField.LAST_PLAYED_AT]: "Last Played",
};

const SortArrow: FC<{ order: SearchSuiteConfigurationSortOrder }> = ({ order }) => (
    <svg
        className={`h-5 w-5 ${order === SearchSuiteConfigurationSortOrder.ASC ? "rotate-180 transform" : ""}`}
        fill="none"
        stroke="currentColor"
        viewBox="0 0 24 24"
        ...
        <div className="mx-8 my-8 flex items-center justify-between bg-white">
            ...
            <div className="flex items-center space-x-4">
                ...
                <div className="flex items-center space-x-2">
                    <select
                        value={sortField}
                        onChange={(e) => onSortFieldChange(e.target.value as SearchSuiteConfigurationSortField)}
                        className="h-9 w-32 rounded-md border-2 border-gray-200 pl-2 pr-2 text-sm"
                    >
                        {Object.values(SearchSuiteConfigurationSortField).map(
                            (field: SearchSuiteConfigurationSortField) => (
                    ...
                    <button
                        onClick={() =>
                            onSortOrderChange(
                                sortOrder === SearchSuiteConfigurationSortOrder.ASC
                                    ? SearchSuiteConfigurationSortOrder.DESC
                                    : SearchSuiteConfigurationSortOrder.ASC,
                            )
                        }
